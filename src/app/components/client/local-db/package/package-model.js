angular
	...
	.factory('LocalDb_Package', function(
		...
		LocalDb_Package.fromPackageInfo = function(
			_package,
			release,
			build,
			launchOptions
			...
			packageData.release = _.pick(release, ['id', 'version_number']);
			...
			packageData.launch_options = _(launchOptions)
				...
				.map(function(item) {
					return _.pick(item, ['id', 'os', 'executable_path']);
				})
				.value();
		...
		LocalDb_Package.createForInstall = function(
			_package,
			release,
			build,
			launchOptions
		...
		LocalDb_Package.prototype.isPatching = function() {
			return (
				this.install_state == LocalDb_Package.PATCH_PENDING ||
		...
		LocalDb_Package.prototype.didInstallFail = function() {
			return (
				this.install_state == LocalDb_Package.DOWNLOAD_FAILED ||
		...
		LocalDb_Package.prototype.didUpdateFail = function() {
			return (
				this.update_state == LocalDb_Package.DOWNLOAD_FAILED ||
		...
		LocalDb_Package.prototype.$startUpdate = function(newBuildId) {
			...
			return Api.sendRequest(
				...
				_this.update = LocalDb_Package.fromPackageInfo(
					response.package,
					response.release,
					response.build,
		...
		LocalDb_Package.prototype.$uninstall = function() {
			...
			this._uninstallingPromise = LocalDb.transaction(
				'rw',
				[LocalDb_Game, LocalDb_Package],
				function() {
					...
					return Client_Installer.cancel(_this)
						...
						.then(function() {
							...
							return LocalDb_Package.table()
								.where('game_id')
								.equals(_this.game_id)
								.count()
								.then(function(count) {
