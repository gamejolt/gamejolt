angular
	...
	.service('LocalDb_Sync', function(
		...
		this.check = function() {
			...
			$q
				.all([LocalDb_Game.fetch(), LocalDb_Package.fetch()])
				.then(function(data) {
					var games = data[0];
					var packages = data[1];
		...
		this.processResponse = function(response) {
			...
			var promises = gamePromises
				.concat(packagePromises)
				.concat(updateBuildsPromises);
			return $q.all(promises);
		};

		this.syncGame = function(id, data) {
			return LocalDb.transaction('rw', [LocalDb_Game], function() {
				return LocalDb_Game.fetch(id).then(function(localGame) {
		...
		this.syncPackage = function(id, data) {
			return LocalDb.transaction('rw', [LocalDb_Package], function() {
				return LocalDb_Package.fetch(id).then(function(localPackage) {
					...
					var _launchOptions = _.where(data.launchOptions, {
						game_build_id: localPackage.build.id,
					});

					// Assign so we don't lose fields.
					var newPackage = LocalDb_Package.fromPackageInfo(
						_package,
						_release,
						_build,
						_launchOptions,
