angular
	.module('App.Client.Library')
	.run(function(Client_Library) {
		Client_Library.setupDbWatcher().then(function() {
	...
	.service('Client_Library', function(
		...
		this.setupDbWatcher = function() {
			...
			watchThese.forEach(function(watchInfo) {
				...
				watchInfo.model
					.table()
					.hook('updating', function(mods, key, obj, trans) {
						trans.on('complete', function() {
			...
			return $q.all(fetchers).then(function(data) {
	 ...
	 */
		...
		this.installPackage = function(
			...
			HistoryTick.sendBeacon('game-build-install', _build.id, {
				sourceResource: 'Game',
				sourceResourceId: _game.id,
			});

			var game = LocalDb_Game.fromGame(_game);
			var localPackage = LocalDb_Package.createForInstall(
				_package,
				_release,
				_build,
				_launchOptions,
