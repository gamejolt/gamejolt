	...
	$q, $ocLazyLoad, $window, Form, Api, Growls, Environment, Geo, currencyFilter )
{
	...
	form.onInit = function( scope )
	{
		...
		Loader.load( 'upload' )
			...
			.then( function( payload )
			{
				...
				if ( payload.stripe && payload.stripe.current && payload.stripe.current.legal_entity.additional_owners ) {
					scope.formModel.additional_owners_count = payload.stripe.current.legal_entity.additional_owners.length;
				}
				...
				scope.formState.isComplete = payload.account.is_verified && !payload.stripe.current.verification.due_by;
		...
		scope.helpers.requiresField = function( field )
		{
			...
			return scope.stripeMeta.minimum.indexOf( field ) !== -1
				...
				|| (
					...
					&& scope.stripe.current.verification.fields_needed.indexOf( field ) !== -1
				);
		};
		...
		scope.helpers.removeAdditionalOwner = function( index )
		{
			...
			{
				var regex = new RegExp( 'legal_entity\-additional_owners\-' + oldIndex + '\-' );
				for ( var field in scope.formModel ) {
					if ( regex.test( field ) ) {
						var newField = field.replace( '-' + oldIndex + '-', '-' + newIndex + '-' );
			...
			for ( var i = index + 1; i < scope.formModel.additional_owners_count; ++i ) {
