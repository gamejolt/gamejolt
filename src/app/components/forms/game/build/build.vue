	...
	<AppCardListItem class="game-build-form" force-active :item="model">
		...
		<template #body>
			<AppForm :controller="form">
				...
				>
					<AppJolticon icon="notice" />
					<strong><AppTranslate>Silverlight builds have been deprecated.</AppTranslate></strong>
					<AppTranslate>
					...
					<template v-if="archiveError">
						<AppExpand class="-archive-error" :when="true">
							<div class="alert alert-notice sans-margin-bottom">
								<p>{{ archiveError }}</p>
								<p><AppTranslate>Please re-upload with a valid archive.</AppTranslate></p>
							</div>
						</AppExpand>
					</template>
						...
						>
							...
							<AppExpand :when="!model.primary_file.is_archive">
								<div>
									...
									<p><AppTranslate>It can be launched automatically.</AppTranslate></p>
								</div>
							</AppExpand>
						</fieldset>
