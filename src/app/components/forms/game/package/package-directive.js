...
angular.module( 'App.Forms.Dashboard' ).directive( 'gjFormDashboardGamePackage', function( $state, Form, App, Api, Game_Package, Sellable, Sellable_Pricing, Timezone, ModalConfirm, gettextCatalog )
{
	...
	form.onInit = function( scope )
	{
		...
		Api.sendRequest( '/web/dash/developer/games/packages/save/' + params.join( '/' ) ).then( function( payload )
		{
			...
			if ( scope.method === 'add' ) {
				...
				if ( scope.sellable.type !== 'free' ) {
					...
					scope.originalPricing = Sellable_Pricing.getOriginalPricing( scope.pricings );
					scope.promotionalPricing = Sellable_Pricing.getPromotionalPricing( scope.pricings );

					scope.formModel.price = scope.originalPricing ? scope.originalPricing.amount / 100 : 0;
					...
					if ( scope.promotionalPricing ) {
						...
						scope.formModel.sale_price = scope.promotionalPricing.amount / 100;

						scope.saleStartLocal = moment( scope.promotionalPricing.start ).tz( scope.promotionalPricing.timezone );
						scope.saleEndLocal = moment( scope.promotionalPricing.end ).tz( scope.promotionalPricing.timezone );
					}
		...
		if ( !scope.isLoaded ) {
			...
			{
				ModalConfirm.show( gettextCatalog.getString( 'Are you sure you want to cancel this sale?' ) )
					.then( function()
					{
						scope.isProcessing = true;
						var params = [ scope.formModel.game_id, scope.formModel.id ];
						return Api.sendRequest( '/web/dash/developer/games/packages/cancel-sales/' + params.join( '/' ) );
					} )
