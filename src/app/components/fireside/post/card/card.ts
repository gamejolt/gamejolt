...
export default class AppPostCard extends Vue {
	...
	calcData() {
		const parentWidth = Ruler.width(this.$el.parentElement as HTMLElement);

		const cardWidth = Math.floor(parentWidth);
		const cardHeight = Math.floor(parentWidth / this.aspectRatio);
