...
function isNewMessage(message: ChatMessage) {
	const newCount = chat.value.notifications[room.value.id];
	if (newCount === 0) {
		return false;
	}

	// Use messages, don't consider queued messages here.
	const position = messages.value.indexOf(message);
	return messages.value.length - position === newCount;
}
