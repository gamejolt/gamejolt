angular
	.module('App.Views')
	.controller('Discover.Games.ViewCtrl', function(
		$scope,
		$rootScope,
		$state,
		$transition$,
		...
		this.init = function(payload) {
			...
			this.primaryScoreTable = payload.primaryScoreTable
				? new Game_ScoreTable(payload.primaryScoreTable)
				: null;
			...
			if (this.userPartnerKey) {
				this.partnerLink =
					Environment.baseUrl +
					$state.href('discover.games.view.overview', {
						id: this.game.id,
						slug: this.game.slug,
		...
		function processRatingPayload(payload) {
			_this.userRating = payload.userRating
				? new Game_Rating(payload.userRating)
				: null;
