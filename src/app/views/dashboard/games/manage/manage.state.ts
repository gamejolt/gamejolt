...
import { VuexModule, VuexStore, VuexMutation, VuexAction } from '../../../../../lib/gj-lib-client/utils/vuex';
...
export class RouteStore extends VuexStore<RouteStore, Actions, Mutations>
{
	...
	async publish()
	{
		const result = await ModalConfirm.show( Translate.$gettext( 'dash.games.overview.publish_confirmation' ) );
		if ( !result ) {
			return;
		}
	...
	async hide()
	{
		const result = await ModalConfirm.show( Translate.$gettext( 'Are you sure you want to hide your game page?' ) );
		if ( !result ) {
			return;
		}
	...
	async cancel()
	{
		const result = await ModalConfirm.show( Translate.$gettext( 'Are you sure you want to cancel your game?' ) );
		if ( !result ) {
			return;
		}
	...
	async uncancel()
	{
		const result = await ModalConfirm.show( Translate.$gettext( 'Are you sure you want to uncancel your game?' ) );
		if ( !result ) {
			return;
		}
