...
export default class AppStickerTarget extends Vue {
	...
	get stickers() {
		...
		return [...this.controller.stickers.value].sort((a, b) => a.id - b.id);
	}

	get isShowingStickers() {
		return (
			this.controller.shouldShow.value &&
			this.controller.isInview.value &&
	...
	created() {
		...
		watch(this.controller.shouldLoad, shouldLoad => {
			if (shouldLoad) {
				this.loadStickers();
			}
		});
	...
	async onIsShowingStickersChange() {
		...
		if (this.isShowingStickers) {
			this.controller.newStickers.value = [];
		}
	}

	private async loadStickers() {
		this.controller.hasLoadedStickers.value = true;
		...
		this.controller.stickers.value = StickerPlacement.populate(stickers);
	}
	...
	async onInview() {
		...
		if (this.queuedInview) {
			this.controller.isInview.value = true;
	...
	onOutview() {
		this.queuedInview = false;
		this.controller.isInview.value = false;
	}
	...
	getStickerAnimationDelay(placement: StickerPlacement) {
		...
		if (this.controller.newStickers.value.includes(placement)) {
			return '0.1s';
		}
