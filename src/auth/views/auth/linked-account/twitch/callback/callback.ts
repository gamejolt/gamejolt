...
@Component({})
export default class RouteAuthLinkedAccountTwitchCallback extends Vue
{
	@BeforeRouteEnter()
	...
	routed()
	{
		if ( !this.$payload.success ) {
			...
			if ( this.$payload.reason && this.$payload.reason === 'no-email' ) {
				Growls.error( {
					sticky: true,
					message: this.$gettext( `Your Twitch account did not return an email address. Make sure you have verified it with Twitch.` ),
				} );
			}
			else if ( this.$payload.reason && this.$payload.reason === 'duplicate-email' ) {
				Growls.error( {
					sticky: true,
					message: this.$gettext( `The email address on this Twitch account is already in use. Perhaps you already have an account?` ),
				} );
			}
			else {
				Growls.error( {
